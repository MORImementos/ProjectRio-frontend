...
const communityUpdate = Community.pick({
    id: true,
    name: true,
    type: true,
    private: true,
...
export const load = async ({ event, cookies, fetch }) => {
    ...
    const form = await superValidate(event, communityUpdate);
    return {
        form
    };
};
...
export const actions = {
    default: async ({ request, fetch }) => {
        const form = await superValidate(request, communityUpdate);
        ...
        const response = await fetch(BACKEND + COMMUNITY_ENDPOINTS.COMMUNITY_UPDATE, {
