          ...
          {#if withinLastDay(games.date_time_start)}
            <tr>
              <td class="player-link">
                <a class="player" href={`/modes/player/${games.away_user}`}>{games.away_user}</a>
              </td>
              <td>{games.away_score}</td>
              <td>{games.home_score}</td>
              <td class="player-link">
                <a class="player" href={`/modes/player/${games.home_user}`}>{games.home_user}</a>
              </td>
              <!-- convert stadium id num to string -->
              <td>{stadiums[games.stadium]}</td>

              <!-- convert tag id num to string -->
              <td class="game-mode"><a href={`/modes/${tagsetsData.find(tagset => tagset.id === games.game_mode)?.name}`}/ladder>{tagsetsData.find(tagset => tagset.id === games.game_mode)?.name || ''}</a></td>
